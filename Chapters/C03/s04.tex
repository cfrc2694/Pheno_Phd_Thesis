...
The training and evaluation of the Neural Networks and BDTs were carried out in a high-performance computing environment. Specifically, an Nvidia A100 GPU was used. The canonical \texttt{PyTorch}~\parencite{paszke2019} deep learning framework was employed for configuring, training, and evaluating the neural networks. PyTorch is well-regarded for its flexibility and performance in deep learning applications.

For the neural network architectures, we explored three different configurations of fully-connected (FC) layers: Neural Network 1 (NN1) consists of 3 FC layers with widths 32, 64, and 1; Neural Network 2 (NN2) implements 4 FC layers with widths 64, 128, 128, and 1; and Neural Network 3 (NN3) utilizes a deeper architecture with 5 FC layers of widths 64, 128, 512, 128, and 1. These architectures represent increasing levels of complexity, allowing us to assess the trade-off between model depth and classification performance.
...
\begin{table}
    ...
    \hline
    \end{tabular}
    \caption{Comparison of training and testing accuracies with corresponding training times for the BDT and three neural network architectures. All models achieve excellent classification performance with test accuracies exceeding 99.9\%, while the BDT demonstrates a substantial computational advantage with training times of only 6 seconds compared to approximately 2 hours for the neural networks.}\label{tab:ml_results}
    \centering
\end{table}

The outcomes of our model training and evaluation are presented in Tab.~\ref{tab:ml_results}. As shown in the table, all models achieve exceptional classification performance, with test accuracies exceeding 99.9\%. The BDT achieves a test accuracy of 99.93\%, while the three neural network architectures achieve marginally higher accuracies ranging from 99.97\% to 99.98\%. However, this minimal improvement in classification performance comes at a significant computational cost: the neural networks require training times between 1 hour 58 minutes and 2 hours 32 minutes, compared to only 6 seconds for the BDT—representing a speedup of over three orders of magnitude.
