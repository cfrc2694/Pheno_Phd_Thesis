...
\begin{center}
    \includegraphics[width=\linewidth]{Images/SLQ_Cross_Section.pdf}
...
\begin{center}
    \includegraphics[width=.9\linewidth]{Images/VLQ_BranchingRatio.pdf}
...
\begin{align}
    \abs{\mathcal{M}_R}^2&=\abs{\mathcal{M}({\tau^+_R \tau^-_R})}^2 + \abs{\mathcal{M}({\tau^+_L \tau^-_R})}^2,\\
    \abs{\mathcal{M}_L}^2&=\abs{\mathcal{M}({\tau^+_L \tau^-_L})}^2 + \abs{\mathcal{M}({\tau^+_R \tau^-_L})}^2.
\end{align}
...
\begin{center}
    \includegraphics[width=\linewidth]{Images/P_vlQ_tau_minus_vs_eta.pdf}
...
\end{equation}
Figure~\ref{fig:polarization_asymmetry_avg} shows the average polarization asymmetry $\mathcal{P}_{\tau^-}^{\text{avg}}$ of the $\tau$ lepton as a function of its pseudorapidity $\eta$ for different values of the $\beta_R^{b\tau}$ parameter. 

\begin{center}
    \includegraphics[width=\linewidth]{Images/P_vlQ_tau_minus_vs_eta_avg.pdf}
...
\end{center}
% The cross section decreases with increasing $U_1$ mass, as expected, but the relative weight of polarized contributions shifts due to the chiral structure of the couplings. For instance, a negative $\beta_R^{b\tau}$ enhances RH $\tau$ production, reducing the LH/RH asymmetry compared to the $\beta_R^{b\tau} = 0$ case. This effect is measurable in precision analyses of $\tau$-pair events at the LHC.  
...
% To further understand the role of $\beta_R^{b\tau}$ at colliders, Figure~\ref{fig:cross_section} shows the cross-section for single-${\rm vLQ}$ (s${\rm vLQ}$), double-${\rm vLQ}$ (d${\rm vLQ}$), and non-resonant (non-res) production, as a function of mass and for a fixed coupling $g_{U} = 1.8$, assuming $\mathrm{p}\,\mathrm{p}$ collisions at $\sqrt{s} = 13$ $\tev$. We note that this benchmark scenario with $g_{U}=1.8$ results in a ${\rm vLQ}\to{\rm b}\tau$ decay width that is $<$5\% of the ${\rm vLQ}$ mass, for mass values from 250 $\gev$ to 2.5 $\tev$. In the Figure, we observe that, since d${\rm vLQ}$ production is mainly mediated by events from quantum chromodynamic processes, the choice of $\beta_R^{b\tau}$ does not affect the cross-section. However, for  s${\rm vLQ}$ production, a non-zero value for $\beta_R^{b\tau}$ increases the cross-section by about a factor of 2 and by almost one order of magnitude in the case of non-res production. These results shown in Figure~\ref{fig:branching_ratios} are easily understood by considering the diagrams shown in Figure~\ref{fig:feynmp-prod-channels}. The ${\rm vLQ}$ mass value where the s${\rm vLQ}$ production cross-section exceeds the d${\rm vLQ}$ cross-section depends on the choice of $g_U$. 
 



% As noted in section~\ref{sec:intro}, we study the role of a $\zb'$ boson in $\mathrm{p}\,\mathrm{p}\to\tau\tau$ production. The presence of a $\zb'$ boson in ${\rm vLQ}$ models has been justified in various papers, for example, in~\parencite{Baker:2019sli}. The argument is that minimal extensions of the SM which include a massive gauge $U_1$ LQ, uses the gauge group $SU(4)\times SU(3)^{\prime}\times SU(2)_L \times U(1)_{T_R^3}$. Such an extension implies the presence of an additional massive boson, $\zb^{\prime}$, and a color-octet vector, $G'$, arising from the spontaneous symmetry breaking into the SM \marginpar{Naively, the LQs are associated to the breaking of $SU(4)\to SU(3)_{[4]}\times U(1)_{B-L}$, the $G'$ arises from $SU(3)_{[4]}\times SU(3)'\to SU(3)_c$, and the $Z'$ comes from the breaking of $U(1)_{B-L}\times U(1)_{T_R^3}\to U(1)_Y$. Notice that the specific pattern of breaking, and the relations between the masses and couplings, are connected to the specific scalar potential used.}.  The $\zb'$ in particular can play an important role in the projected ${\rm vLQ}$ discovery reach, as it can participate in $\mathrm{p}\,\mathrm{p}\to\tau\tau$ production by s-channel exchange, both resonantly and as a virtual mediator. To study the effect of a $\zb'$ on the $\mathrm{p}\,\mathrm{p}\to\tau\tau$ production cross-sections and kinematics, we extend our benchmark Lagrangian in Eq.~(\ref{eq:BasicLagrangian}) with further non-renormalizable terms involving the $\zb'$. Accordingly, we assume the $\zb'$ only couples to third-generation fermions. Our simplified model is thus extended by:
% \begin{eqnarray}
...
%     \end{subfigure}
%     \caption{$\tau \tau$ cross-section as a function of the ${\rm vLQ}$ mass for different values of $g_U$ and $g_{\zb^{\prime}}$. The estimates are performed at $\sqrt s=13 \tev$, $\beta_R^{b\tau}=0$,  $M_{\zb^{\prime}} = \sqrt{1/2} M_{U}$ (top), and $M_{\zb^{\prime}} = \sqrt{3/2} M_{U}$ (bottom).}
...
%     \end{subfigure}
%     \caption{The relative kinematic interference (RKI), as a function of the reconstructed mass of two taus, for different ${\rm vLQ}$ masses. The studies are performed assuming $\sqrt s=13 \tev$, $\beta_R^{b\tau}=0$, $g_U = 1.0$, $g_{\zb^{\prime}} =1.0$, $M_{\zb^{\prime}} = \sqrt{1/2} M_{U}$ (top), and $M_{\zb^{\prime}} = \sqrt{3/2} M_{U}$ (bottom).
%     }    
...
% \begin{equation}
%     \mathrm{RKI}(m_{\tau\tau})=\frac{1}{\sigma_{{\rm vLQ}+\zb'}}\left[\frac{d\sigma_{{\rm vLQ}+\zb'}}{dm_{\tau\tau}}-\left(\frac{d\sigma_{{\rm vLQ}}}{dm_{\tau\tau}}+\frac{d\sigma_{\zb'}}{dm_{\tau\tau}}\right)\right],
% \end{equation}
% where $\sigma_{X}$ is the production cross-section arising due to contributions from $X$ particles. For example, $\sigma_{{\rm vLQ}+\zb'}$ represents the inclusive cross-section where both virtual ${\rm vLQ}$ and s-channel $\zb'$ exchange contribute. For both cases, we can observe the presence of deep valleys in the RKI curves when $m_{\tau\tau}\to0$, indicating destructive interference between the ${\rm vLQ}$ and the $\zb'$ contributions. This interference generates a suppression of the differential cross-section for lower values of $m_{\tau\tau}$ and, therefore, in the integrated cross-section. 
 
% The observed interference effects are consistent with detailed studies on resonant and non-res $\mathrm{p}\,\mathrm{p}\to{\rm t} \bar{{\rm t}}$ production, performed in reference~\parencite{Djouadi:2019cbm}.
