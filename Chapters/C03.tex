...
For these reasons, this section focuses on the simplified model containing only the scalar leptoquark $\tilde{S}_1$. The most general renormalizable Lagrangian contains the couplings:
\begin{equation}
    \mathcal{L}_{\text{Yukawa}} \supset \tilde{y}_{ij} \overline{d}_R^{C\,i} \tilde{S}_1 e_R^j + \tilde{z}_{ij} \overline{u}_R^{C\,i} \tilde{S}_1^* u_R^j + \text{h.c.}
    \label{eq:yukawa}
\end{equation}
where $\tilde{y}, \tilde{z}$ are complex Yukawa matrices, with $\tilde{z}^{RR}$ antisymmetric due to Fermi statistics.
