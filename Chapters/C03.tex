...
\begin{center}
    ...
    \captionof{figure}{Top: The $\textrm{vLQ}\to\textrm{b}\tau$ and $\textrm{vLQ}\to\textrm{t}\nu$ branching ratios for $\beta_{R} = 0$ (solid lines) and $\beta_{R} = -1$ (dashed lines). Bottom: Signal cross-section as a function of the $\textrm{vLQ}$ mass, for $\sqrt{ s}=13 \tev$, with $g_U=1.8$. We show single, pair, and non-resonant production, for $\beta_R^{b\tau}=-1,\,0$ in solid and dashed lines, respectively.}\label{fig:branching_ratios}
\end{center}
