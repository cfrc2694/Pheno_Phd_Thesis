...
\end{equation}
The expected behavior differs under the signal-plus-background ($H_1$) and background-only ($H_0$) hypotheses. But, the expressions for $\langle t \rangle$ and $\sigma^2(t)$ under both hypotheses follow from the properties of Poisson-distributed random variables $n_i$ and the linearity of expectation.

For any hypothesis, the expectation value of $t$ is given by
\begin{equation}
\langle t \rangle = \sum_i \left[2s_i - 2w_i \langle n_i \rangle\right],
\end{equation}
since $\langle n_i \rangle$ depends on the hypothesis. The variance derivation uses the facts that:
\begin{itemize}
    \item Bins are independent: $\sigma^2(\sum_i X_i) = \sum_i \sigma^2(X_i)$
    \item For constants $a,c$: $\sigma^2(aX + c) = a^2\sigma^2(X)$
    \item For Poisson $n_i$: $\sigma^2(n_i) = \langle n_i \rangle = \lambda_i$
\end{itemize}
The variance of each bin's contribution is:  
\begin{equation}
\sigma^2(2s_i - 2w_i n_i) = 4w_i^2 \sigma^2(n_i) = 4w_i^2 \langle n_i \rangle,
\end{equation}
where $\langle n_i \rangle = \lambda_i$ depends on the hypothesis.
