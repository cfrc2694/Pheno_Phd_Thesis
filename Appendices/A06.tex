...
\end{pmatrix},
\end{equation}
and assuming real parameters for clarity, we compute the diagonalization:
...
\end{pmatrix}.
\end{align*}
First, computing $U_L H_L$:
\begin{align*}
U_L H_L &= \begin{pmatrix}
\cos\theta_L & -\sin\theta_L \\
\sin\theta_L & \cos\theta_L
\end{pmatrix}
\begin{pmatrix}
...
\end{pmatrix} \\
&= \begin{pmatrix}
m_L^2 \cos\theta_L - m_{\chi} m_L \sin\theta_L & m_L m_{\chi} \cos\theta_L - (m_R^2 + m_{\chi}^2) \sin\theta_L \\
m_L^2 \sin\theta_L + m_{\chi} m_L \cos\theta_L & m_L m_{\chi} \sin\theta_L + (m_R^2 + m_{\chi}^2) \cos\theta_L
\end{pmatrix}.
\end{align*}

Then multiply by $U_L^\dagger$, and with $\tilde{H}_L\equiv U_L H_L U_L^\dagger$ we have that the diagonal elements are:
\begin{align*}
(\tilde{H}_L)_{11} &= (m_L^2 \cos\theta_L - m_{\chi} m_L \sin\theta_L)\cos\theta_L \\
&\quad + [m_L m_{\chi} \cos\theta_L - (m_R^2 + m_{\chi}^2) \sin\theta_L](-\sin\theta_L) \\
&= m_L^2 \cos^2\theta_L - m_L m_{\chi} \sin\theta_L \cos\theta_L \\
&\quad - m_L m_{\chi} \cos\theta_L \sin\theta_L + (m_R^2 + m_{\chi}^2) \sin^2\theta_L \\
&= m_L^2 \cos^2\theta_L + (m_R^2 + m_{\chi}^2) \sin^2\theta_L - 2m_L m_{\chi} \sin\theta_L \cos\theta_L,
\end{align*}

\begin{align*}
(\tilde{H}_L)_{22} &= (m_L^2 \sin\theta_L + m_{\chi} m_L \cos\theta_L)\sin\theta_L \\
&\quad + [m_L m_{\chi} \sin\theta_L + (m_R^2 + m_{\chi}^2) \cos\theta_L]\cos\theta_L \\
&= m_L^2 \sin^2\theta_L + m_L m_{\chi} \sin\theta_L \cos\theta_L \\
&\quad + m_L m_{\chi} \sin\theta_L \cos\theta_L + (m_R^2 + m_{\chi}^2) \cos^2\theta_L \\
&= m_L^2 \sin^2\theta_L + (m_R^2 + m_{\chi}^2) \cos^2\theta_L + 2m_L m_{\chi} \sin\theta_L \cos\theta_L.
\end{align*}

While, the off-diagonal element is:
\begin{align*}
(\tilde{H}_L)_{12} &= (m_L^2 \cos\theta_L - m_{\chi} m_L \sin\theta_L)\sin\theta_L \\
&\quad + [m_L m_{\chi} \cos\theta_L - (m_R^2 + m_{\chi}^2) \sin\theta_L]\cos\theta_L \\
&= m_L^2 \sin\theta_L \cos\theta_L - m_L m_{\chi} \sin^2\theta_L \\
&\quad + m_L m_{\chi} \cos^2\theta_L - (m_R^2 + m_{\chi}^2) \sin\theta_L \cos\theta_L \\
&= m_L m_{\chi} (\cos^2\theta_L - \sin^2\theta_L) + [m_L^2 - (m_R^2 + m_{\chi}^2)] \sin\theta_L \cos\theta_L.
\end{align*}

Requiring this equals $\text{diag}(m_f^2, m_F^2)$ gives the system:
\begin{align*}
m_L^2 \cos^2\theta_L + (m_R^2 + m_{\chi}^2) \sin^2\theta_L - 2m_L m_{\chi} \sin\theta_L \cos\theta_L &= m_f^2, \\
m_L^2 \sin^2\theta_L + (m_R^2 + m_{\chi}^2) \cos^2\theta_L + 2m_L m_{\chi} \sin\theta_L \cos\theta_L &= m_F^2, \\
m_L m_{\chi} (\cos^2\theta_L - \sin^2\theta_L) + [m_L^2 - (m_R^2 + m_{\chi}^2)] \sin\theta_L \cos\theta_L &= 0.
\end{align*}

Adding and subtracting appropriate combinations of these equations yields the simplified system:
\begin{align*}
...
m_{\chi}^2 &= \frac{(m_F^2 - m_f^2)^2 \sin^2 2\theta_L}{4 m_L^2}. \label{eq:m_chi_exact_detail}
\end{align}
